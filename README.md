...
정확한 `VERSION-PACKAGE_RELEASE` 마커(예: `1.0.0-2`)로 이를 강제한다. `v1.0.0-2` 같은 태그는
애플리케이션 버전 `1.0.0`을 바꾸지 않고 네이티브 패키징 릴리즈 2를 게시하므로 패키징만 수정할 때 쓴다.
앱은 X11을 직접 쓰며 Wayland에서는 XWayland를 거친다.
