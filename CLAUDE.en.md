...
- Row icons in the file panes are drawn geometry, not emoji. 📁/📄 look fine on Windows and
  macOS, but the only bundled font is Inter, Inter has no emoji, and a machine without an emoji
  font gets whatever fontconfig substitutes — in practice a striped box. A self-contained build
  must not depend on the host having a particular font, for the same reason it must not depend
  on a system library. The geometry lives in `ListingConverters`.
- A cell with no value does not print a zero. `Size` renders `-` for directories and prefixes
  (the `isContainer` overload of `ByteSize.Format`), and Modified is blank for a row with no
  timestamp — a `StringFormat` binding renders null as the value type's default, which is
  `0001-01-01 00:00`, so both go through converters. `FilePaneColumnTests` pins this by
  rendering the real panes headlessly and reading the cells back.
