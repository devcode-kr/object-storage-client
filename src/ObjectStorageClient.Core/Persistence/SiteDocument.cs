...
internal sealed record StoredSite
{
    ...
    // The defaults must match ConnectionProfile's: a file missing one of these should degrade to
    // the compatible setting, not to `false`. Checksums and chunked encoding especially — turning
    // those on is what makes S3-compatible gateways reject every upload.
